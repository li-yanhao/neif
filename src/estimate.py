...
def estimate_noise_curve(img_ref, img_mov, w: int, T: int, th: int, q: float, bins: int, s: int, f=1):
    ...
    for ch in range(C):
        ...
        # subscale block pairs
        # select q percent of block pairs
        # partition
        # estimate noise

        blks_ref = view_as_windows(img_ref_chnl, (w_up, w_up), step=(1, 1))
        blks_ref = blks_ref[pos_ref[:, 0], pos_ref[:, 1]]

        blks_mov = view_as_windows(img_mov_chnl, (w_up, w_up), step=(1, 1))
        blks_mov = blks_mov[pos_mov[:, 0], pos_mov[:, 1]]
        ...
        blks_ref_filtered, blks_mov_filtered = M.filter_block_pairs(blks_ref, blks_mov, T, q)
        blks_ref_in_bins, blks_mov_in_bins = M.partition_blocks(blks_ref_filtered, blks_mov_filtered, w, bins)
        
        for b in range(bins):
            blks_ref = blks_ref_in_bins[b]
            blks_mov = blks_mov_in_bins[b]

            variance = M.compute_variance_from_pairs(blks_ref, blks_mov, T)
            intensity = (blks_ref.mean() + blks_mov.mean()) / 2
        ...
        #     img_ref_chnl, img_mov_chnl, pos_ref_filtered, pos_mov_filtered, w_up, bins) # (bins, N, 2)
        ...
        #     blks_ref_in_bin = blks_ref[pos_ref[:, 0], pos_ref[:, 1]] # (N, w_up, w_up)
        #     blks_mov_in_bin = blks_mov[pos_mov[:, 0], pos_mov[:, 1]] # (N, w_up, w_up)
            

        #     if f > 1:
        #         blks_ref_in_bin = view_as_blocks(blks_ref_in_bin, (1, f, f)).squeeze() # (N, w, w, f, f)
        #         # blks_ref = np.transpose(blks_ref, (0, 3, 4, 1, 2)) # (N, f, f, w, w)
        #         blks_ref_in_bin = blks_ref_in_bin.transpose((0, 3, 4, 1, 2)).reshape(-1, w, w) # (N', w, w)

        #         blks_mov_in_bin = view_as_blocks(blks_mov_in_bin, (1, f, f)).squeeze() # (N, w, w, f, f)
        #         blks_mov_in_bin = blks_mov_in_bin.transpose((0, 3, 4, 1, 2)).reshape(-1, w, w) # (N', w, w)
